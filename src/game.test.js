...
it("test ship IO",()=>{
    ...
    const myShip=Ship(3)
    myBoard.place(myShip,"vertical",0,0)
...
// it("test board placement of ships",()=>{
    ...
    it("hit ships correctly",()=>{
        const myShip=Ship(3)
        const myBoard=Board(6)
        myBoard.place(myShip,"horizontal",2,0)
        myBoard.recieveAttack(2,0)
        console.log(myShip)
        console.log(myBoard.boardActual)
...
it("hit all ships",()=>{
    const myBoard=Board(6)
    myBoard.place(Ship(2),"horizontal",2,0)
    myBoard.recieveAttack(2,0)
    myBoard.recieveAttack(3,0)
    myBoard.place(Ship(2),"vertical",2,1)
...
it("allow player to hit enemy waters/ships(also sink 'em) and vice versa",()=>{
    ...
    myBoard.place(Ship(2),"vertical",0,0)
    const player=new Player(myBoard)

    const enemyBoard=Board(3)
    enemyBoard.place(Ship(2),"horizontal",0,0)
    const comp=new Player(enemyBoard)
...
it("check aiPlayer hits",()=>{
    ...
    aiBoard.place(Ship(2),"horizontal",0,0)
